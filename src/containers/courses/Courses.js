...
export class Courses extends React.Component {
  ...
  componentDidMount() {
    ...
    this.courseNotifier.subscribe((data) => {
      ...
      if (data && data.added) this.fetchCourses();
    });
  }
  ...
  deleteCourse(id) {
    axios.delete(`/api/courses/${id}`).then((result) => {
      if (result.data.deleted) this.setState({ refresh: true });
    });
  }
              ...
              <Route
                ...
                component={() => (
                  <CourseList
                    ...
                    removeCourse={(id) => this.deleteCourse(id)}
                  ></CourseList>
                )}
              />
